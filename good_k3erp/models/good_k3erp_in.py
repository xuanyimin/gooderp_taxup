...
class GoodK3ErpIn(models.Model):
    ...
    def createstockin(self, conn, line, excel, worksheet, colnames, number, billno):
        ...
        for i in excel:
            ...
            i[u'负责人_FName'] = i[u'验收_FName'] = i[u'保管_FName'] = i[u'业务员_FName']= user_name
    ...
    def exp_k3_goods(self,order = False):
        for line in self.line_ids:
            if not line.is_verified:
                raise UserError(u'发票%s未核验！请确认'%line.name)
        ...
        tax_type = self.k3_sql.tax_category_ids
        for key in colnames:
            worksheet.write(0,j,key)
            j += 1
        for invoice in self.line_ids:
    ...
    def search_groups_name(self, conn, line):
        ...
        if groups_id:
            ...
            raise UserError('请到K3系统增加计量单位%s.%s' % (line.product_unit, line.order_id.name))
        return groups_name
